...
export function LoginPage() {
  ...
  return (
    <div className="page-shell grid min-h-screen place-items-center p-4">
      <Card className="grid w-full max-w-5xl overflow-hidden md:grid-cols-[1.05fr_0.95fr]">
        <div className="bg-emerald-950 p-8 text-white md:p-12">
          <div className="mb-12 grid h-14 w-14 place-items-center rounded-lg bg-amber-300 text-2xl font-black text-emerald-950">B</div>
          <h1 className="max-w-lg text-4xl font-black leading-tight md:text-5xl">{t('appName')}</h1>
          <p className="mt-5 max-w-md text-lg text-emerald-50">{t('demoHint')}</p>
          <div className="mt-8 grid grid-cols-2 gap-3">
            <div className="rounded-lg bg-white/10 p-4">
              <p className="text-3xl font-black">20</p>
              <p className="text-sm text-emerald-100">{t('products')}</p>
            </div>
            <div className="rounded-lg bg-white/10 p-4">
              <p className="text-3xl font-black">FR/AR</p>
              <p className="text-sm text-emerald-100">{t('dashboard')}</p>
            </div>
          </div>
        </div>
        <form className="grid gap-5 p-8 md:p-12" onSubmit={handleSubmit}>
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-sm font-bold text-emerald-800">{isCreatingManager ? t('newAccount') : t('login')}</p>
              <h2 className="text-3xl font-black text-emerald-950">{isCreatingManager ? t('createUserAccount') : t('signIn')}</h2>
            </div>
          ...
          <div className="grid gap-3 rounded-lg bg-emerald-50 p-4 text-sm text-emerald-950">
